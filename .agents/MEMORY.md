...
- **Backup location changed (2026-09-02):** DB dumps now go to `$WORKSPACE_BASE/backups/<branch>/` (OUTSIDE the app dir) — was `$APP_DIR/backups/predeploy/` (web-accessible risk + app dir bloat). rollback.sh searches the new location across branches. Old dumps at the old path (if any) are NOT auto-moved — copy them manually if needed.
