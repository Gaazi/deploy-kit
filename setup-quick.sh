...
while IFS='=' read -r key val; do
  ...
  case "$key" in
    ...
    PRE_DEPLOY_HOOK)   PRE_DEPLOY_HOOK="$val" ;;
    POST_DEPLOY_HOOK)  POST_DEPLOY_HOOK="$val" ;;
    ...
    NOTIFY_ON_SUCCESS)   NOTIFY_ON_SUCCESS="$val" ;;
...
PRE_DEPLOY_HOOK="$(esc "$PRE_DEPLOY_HOOK")"
POST_DEPLOY_HOOK="$(esc "$POST_DEPLOY_HOOK")"
...
NOTIFY_ON_SUCCESS="$(esc "$NOTIFY_ON_SUCCESS")"
